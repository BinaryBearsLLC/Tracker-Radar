...
def main():
    work = Path(os.environ['RUNNER_TEMP']) / 'tracker-signing'
    ...
    try:
        ...
        run('xcrun', 'notarytool', 'store-credentials', 'tracker-radar-release', '--key', str(api_key),
    ...
    with open(os.environ['GITHUB_ENV'], 'a') as env:
        for key, value in {'MACOS_SIGN_IDENTITY': identity, 'SIGNING_KEYCHAIN': keychain,
                           'NOTARY_PROFILE': 'tracker-radar-release', 'SIGNED_MACOS': '1'}.items():
