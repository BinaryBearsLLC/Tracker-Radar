...
- macOS: new Universal build with local Python 3.13.14/Tk; ARM64 and x86_64 (Rosetta) packaged smoke passed. All 54 Mach-O files include both architectures. Strict/deep Developer ID signature verification passed. The app and final branded DMG were separately accepted by Apple, stapled and validated; Gatekeeper reports `Notarized Developer ID` for both. The app mounted from the DMG passed ARM64 and Rosetta smoke tests. Finder layout, volume icon, background, Applications link and BinaryBears website link metadata were checked.
...
The public 1.0.0 Mac ZIP has been replaced with the locally Developer ID signed and notarized Universal DMG of the same tagged application source, built using Python 3.13.14. Both the app and DMG carry validated notarization tickets. Windows and Linux downloads remain the original tagged CI artifacts. Ordinary CI Mac ZIPs are ad-hoc signed; release tags and manually selected signed builds use the complete app-and-DMG signing/notarization workflow. Final package hashes are recorded below.

## Limits

A smoke test verifies startup, Tk, the bundled runtime and spawned worker/result queue. It is not a full manual desktop test. Physical Windows x64 and Intel Mac visual QA, full screen-reader testing, Windows Authenticode signing and testing every supported OS version are not established. Linux requires glibc 2.36+ and a graphical X11/XWayland session.
...
8732e8322ed1c8924cc42cf169bf019c5f1f0475e9e737deae023a1c771a877b  Tracker-Radar-1.0.0-macOS-Universal.dmg
```

## Repository privacy audit

Tracked content and reachable Git history passed Gitleaks 8.30.1 and the dedicated private-key/home-path scanner. No owner personal paths, private keys or credential values were found in those scans. Public release archives and hosted workflow logs were also checked for owner paths and private-key material. Git attribution uses the public BinaryBears company identity. Corporate signing identity and Apple Team ID are expected public certificate metadata. Upstream runtime libraries retain their original third-party build metadata.

Signing inputs remain outside the repository; six encrypted Actions secrets are configured. The runner uses a temporary keychain and cleans it after signing. CI scans each push and pull request. This evidence describes the checks performed, not a guarantee that an automated scanner can identify every possible sensitive string.
